...
**4. 保真靠对账，不靠模型自觉。** 建模 → 语义 fragment → 组装 → 浏览器校验，多环互相拦截：建模层的引用幻觉和数字错，被 fragment 层**回源核对**抓出；响应式、连线和交互问题，被真实浏览器回归抓出。（真实战绩：一次运行同时抓出了建模层把数字取成较晚的重复值、引用了不存在的源块、以及原文档自身"208 台 vs 146+56"的内部矛盾。）保真不是靠某一层不犯错，是靠层与层之间对账。
...
3. **分视图编码**（模型·并行）— 每视图一个 agent 输出语义节点、关系和布局意图，每个内容节点挂溯源锚；流程边只声明起止节点，不手写 SVG 坐标。
4. **确定性组装** — 注入共享视觉、响应式双栏、动态连线、模式切换和溯源交互，产出自包含单文件 HTML。
5. **真实浏览器校验** — 在 1440 / 1280 / 1024 / 768 多视口检查截图、调宽、模式、鼠标与键盘溯源、横向溢出和连线连续性。

细节见 [PIPELINE.md](PIPELINE.md)。

### fragment v2：结构归模型，几何归运行时

流程 fragment 使用一组最小语义标记：`data-flow` 定义流程作用域，`data-node-id` 定义节点，`data-from` / `data-to` 定义关系，`data-source-blocks` 保留到原文的 source map。agent 仍负责真正的信息重编码——选什么图、哪些元素同组、顺序与泳道如何组织；组装器统一负责节点视觉、线路路由、点击 / 键盘反馈和响应式重排。
