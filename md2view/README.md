...
**4. 保真靠对账，不靠模型自觉。** 建模 → 语义 fragment → 候选组装 → 浏览器晋升，多环互相拦截：决策表每行和 checkbox 每项都有稳定原子锚点，弱模型不能再用一条总结吞掉整块；响应式、视觉密度、连线和交互问题由真实浏览器回归抓出。只有全部通过，候选才会原子替换最终 HTML；失败时保留上一个可用版本。
...
最终交付形态是**自适应双栏同步阅读器**：左栏原文、右栏重组，栏宽可拖动并记忆，窄屏切单栏；顶部可切模式，滚动与点击都能按源块锚定联动。右栏用“节点骨架 + 关系 + 可溯源 facts”做到决策完备，左栏只承担核证与细节下钻；流程线由运行时按真实位置绘制并在栏宽变化后重算，视图用紧凑信息行和事实网格消除装饰性空洞。
...
1. **块切分**（确定性）— 切成带 id 的 source blocks，并为表格行、checkbox 项生成稳定 `sourceUnits`。
2. **语义建模**（模型）— 抽概念结构 → `views.json`，逐个结构化原子用 `sourceUnitId` 落到独立节点或 fact。
3. **分视图编码**（模型·并行）— 每视图一个 agent 输出语义节点、关系和布局意图；流程边只声明起止节点，不手写 SVG 坐标。
4. **确定性候选组装** — 校验语义与 fragment 合同，注入共享视觉、双栏、动态连线、模式切换和溯源交互。
5. **真实浏览器验收与晋升** — 在 1440 / 1280 / 1024 / 768 检查视觉密度、调宽、模式、溯源和连线；全部通过才原子替换最终 HTML，不包含手机适配。

细节见 [PIPELINE.md](PIPELINE.md)。

### fragment v2：结构归模型，几何归运行时

流程 fragment 使用一组最小语义标记：`data-flow` 定义作用域，`data-node-id` 定义节点，`data-from` / `data-to` 定义关系，`data-source-blocks` 保留父块 source map，`data-source-unit` 对账决策行和验收项。`concept=matrix` 使用无流程边的 `data-layout="matrix"`；流程线由运行时按真实节点位置路由，关系标签也会做碰撞避让。

旧的 `reader.html` 是自包含快照，可继续打开。旧 fragments 需要按 v2 合同重新生成后，才能获得动态连线、自适应布局和新交互；项目不维护长期双轨兼容层。
...
最终出口是 `scripts/build_reader.py`；直接运行 `assemble_split.py` 只允许生成 `*.candidate.html` 调试候选，不能绕过浏览器门禁覆盖最终文件。
...
*md2view 继承 md2html 的哲学（md 为源、溯源、反 AI slop），但重新划分了职责：AI 负责信息结构和构图意图，确定性运行时负责共享视觉、几何与交互；保真靠 source map 和多环校验。*
