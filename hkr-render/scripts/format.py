...
# ── 图片尺寸缓存（key: 相对路径，value: (width, height)）───────────────
_IMAGE_DIMENSIONS: dict[str, tuple[int, int]] = {}
GALLERY_FULL_WIDTH_RATIO = 2.2
GALLERY_IMG_STYLE = "width:100%;height:auto;display:block;border-radius:8px;margin:0"
...
GALLERY_THEMES = [
    # 精品集（2026-08 精简：34 → 7，其余主题已删除，可从 git 历史找回）
    "hanzhang",    # 签名主题（默认）：克制现代，靛蓝强调
    "newspaper",   # 报纸：严肃深度长文
    "github",      # 开发者风：技术文章
    "magazine",    # 杂志：大留白品质长文
    "ink",         # 墨韵：极简黑白
...
def _get_html_attr(attrs: str, name: str) -> str:
    m = re.search(rf'\b{name}\s*=\s*(["\'])(.*?)\1', attrs, flags=re.IGNORECASE | re.DOTALL)
    return m.group(2) if m else ""


def _image_dimensions(src: str, input_dir: Path) -> tuple[int, int] | None:
    """读取本地图片尺寸；外链或不可读图片返回 None。"""
    if src.startswith(("http://", "https://", "//")):
        return None
    if src in _IMAGE_DIMENSIONS:
        return _IMAGE_DIMENSIONS[src]
...
def _copy_html_image_to_output(src: str, input_dir: Path, output_dir: Path | None) -> str:
    """把 HTML <img> 引用的本地相对图片复制到输出目录，返回渲染用 src。"""
    if output_dir is None or src.startswith(("http://", "https://", "//")):
        return src
    img_path = (input_dir / src).resolve()
    if not img_path.exists():
        return src
...
def gallery_item_width_pct(ratio: float, count: int) -> int:
    """为多图组选择不裁剪的单项宽度。

    目标是让同一组图片形成有节奏的图墙：特别宽的横图通栏，
    其他图片最多两列；所有图片都保留原始比例，不使用 object-fit: cover。
    """
    if count <= 1:
        return width_pct_from_ratio(ratio) or 70
    if ratio >= GALLERY_FULL_WIDTH_RATIO:
        return 100
    return 48


def _style_gallery_img_tag(tag: str) -> str:
    tag = re.sub(r'\s(width|height)=["\'][^"\']*["\']', '', tag)
    if re.search(r'\bstyle\s*=', tag):
        return re.sub(r'style="[^"]*"', f'style="{GALLERY_IMG_STYLE}"', tag)
    tag = tag.rstrip()
    if tag.endswith('/>'):
        return tag[:-2].rstrip() + f' style="{GALLERY_IMG_STYLE}"/>'
    return tag[:-1].rstrip() + f' style="{GALLERY_IMG_STYLE}">'


def _render_gallery_item(img_tag: str, style: str) -> str:
    return f'<section data-gallery-item="1" style="{style}">{img_tag}</section>'


def _render_ordered_gallery_items(items: list[tuple[str, float]]) -> str:
    """按原始顺序渲染：用于步骤图、教程图、漫画分镜等顺序敏感图组。"""
    full_item_style = "box-sizing:border-box;width:100%;padding:4px;line-height:0"
    single_item_style = "box-sizing:border-box;display:inline-block;vertical-align:top;width:48%;padding:4px;line-height:0"

    result: list[str] = []
    for img_tag, ratio in items:
        if len(items) > 1 and ratio >= GALLERY_FULL_WIDTH_RATIO:
            result.append(_render_gallery_item(img_tag, full_item_style))
        else:
            result.append(_render_gallery_item(img_tag, single_item_style))
    return "\n".join(result)


def _render_flexible_gallery_items(items: list[tuple[str, float]]) -> str:
    """允许重排：把尺寸相近的图片配成同一水平栏，减少不规则外轮廓。"""
    full_item_style = "box-sizing:border-box;width:100%;padding:4px;line-height:0"
    row_style = "display:flex;gap:8px;align-items:flex-start;margin:0 4px 8px 4px"
    row_cell_style = "flex:1;min-width:0;line-height:0"
    centered_item_style = "box-sizing:border-box;width:48%;margin:0 auto 8px auto;line-height:0"

    result: list[str] = []
    full_width: list[tuple[str, float]] = []
    pairable: list[tuple[str, float]] = []
    for img_tag, ratio in items:
        if len(items) > 1 and ratio >= GALLERY_FULL_WIDTH_RATIO:
            full_width.append((img_tag, ratio))
        else:
            pairable.append((img_tag, ratio))

    for img_tag, _ratio in full_width:
        result.append(_render_gallery_item(img_tag, full_item_style))

    pairable.sort(key=lambda item: item[1], reverse=True)
    i = 0
    while i < len(pairable):
        if i + 1 >= len(pairable):
            result.append(_render_gallery_item(pairable[i][0], centered_item_style))
            break
        left, right = pairable[i], pairable[i + 1]
        result.append(
            f'<section data-gallery-pack="row" style="{row_style}">'
            f'<section data-gallery-item="1" style="{row_cell_style}">{left[0]}</section>'
            f'<section data-gallery-item="1" style="{row_cell_style}">{right[0]}</section>'
            f'</section>'
        )
        i += 2
...
def render_packed_gallery_items(items: list[tuple[str, float]], *, flexible_order: bool = False) -> str:
    """渲染图片组；展示型图组允许重排，顺序型图组保持原序。"""
    if flexible_order:
        return _render_flexible_gallery_items(items)
    return _render_ordered_gallery_items(items)


def gallery_common_batch_ratio(srcs: list[str]) -> float:
    prefixes = []
    for src in srcs:
        stem = Path(src).stem
        prefixes.append(re.sub(r'[_-]?\d+$', '', stem))
    if not prefixes:
        return 0
    top = max(prefixes.count(p) for p in set(prefixes))
    return top / len(prefixes)


def gallery_is_flexible_order(srcs: list[str], original_alts: list[str]) -> bool:
    """自动判断图片组是否可以为了版面重排。"""
    if len(srcs) < 4:
        return False
    meaningful_alts = [
        alt for alt, src in zip(original_alts, srcs)
        if alt.strip() and alt.strip() != Path(src).stem
    ]
    if meaningful_alts:
        return False
    return gallery_common_batch_ratio(srcs) >= 0.7


def convert_html_galleries(text: str, input_dir: Path, output_dir: Path | None = None) -> str:
    """把 HTML 图片组转换为自适应 gallery HTML。

    input_dir: Markdown 文件所在目录，用于解析相对图片路径。
    支持：
    1. <div align=center>\\n<img .../>\\n<img .../>\\n...</div>
    2. <div align=center>\\n<img .../><br/>\\n<img .../><br/>\\n...</div>

    输出：直接生成不裁剪的自适应图墙，每张图按真实长宽比分配宽度。
    """
    def process_gallery(m: re.Match) -> str:
        gallery_html = m.group(0)
        img_tags = [im.group(1) for im in re.finditer(r'<img\b([^>]*)/?>', gallery_html, re.DOTALL | re.IGNORECASE)]
        if not img_tags:
            return gallery_html

        measured: list[dict[str, object]] = []
        srcs: list[str] = []
        original_alts: list[str] = []
        for tag in img_tags:
            src = _get_html_attr(tag, "src")
            if not src:
                continue
            original_alt = _get_html_attr(tag, "alt")
            alt = original_alt
            if not alt:
                alt = Path(src).stem

            ratio = 1.0
            dims = _image_dimensions(src, input_dir)
            if dims:
                w, h = dims
                ratio = w / h
            measured.append({
                "src": _copy_html_image_to_output(src, input_dir, output_dir),
                "alt": alt,
                "ratio": ratio,
            })
            srcs.append(src)
            original_alts.append(original_alt)
        ...
        for item in measured:
            src = html_utils.escape(str(item["src"]), quote=True)
            alt = html_utils.escape(str(item["alt"]), quote=True)
            ratio = float(item["ratio"])
            img_tag = f'<img style="{GALLERY_IMG_STYLE}" src="{src}" alt="{alt}" data-ratio="{ratio:.2f}"/>'
            items.append((img_tag, ratio))

        flexible_order = gallery_is_flexible_order(srcs, original_alts)
        layout_mode = "flexible" if flexible_order else "strict"
        imgs_html = render_packed_gallery_items(items, flexible_order=flexible_order)
        return (
            f'<section data-container="gallery" data-layout="adaptive" data-order="{layout_mode}">'
...
def width_pct_from_ratio(ratio: float) -> int | None:
    """按图片长宽比映射渲染宽度百分比。

    - ratio >= 1.3：横图 → 100%
    - 0.9 <= ratio < 1.3：近方形 → 75%
    - 0.6 <= ratio < 0.9：轻竖图 → 60%
    - 0.3 <= ratio < 0.6：长竖图 → 45%
    - ratio < 0.3：返回 None（应走 :::longimage）
    """
    if ratio >= 1.3:
        return 100
    if ratio >= 0.9:
...
def measure_and_scale_images(html: str, output_dir: Path) -> str:
    """扫描 HTML 中所有 <img>，按本地图片像素长宽比注入宽度 style。

    - 容器内（gallery / longimage / duo）的 <img> 跳过，由容器自己管。
    - 已有 style 属性的 <img> 跳过（尊重上游注入）。
    - 其他 <img>：打开 output_dir/src 读 size → 映射宽度 → 注入 style。
    - 读图失败（文件缺失/外链/格式不支持）：回退 70% + stderr warn。
    - 比例 < 0.3：回退 45% + stderr 提示考虑套 :::longimage。
    """
    import sys
    try:
        from PIL import Image, UnidentifiedImageError
    except ImportError:
        print("[warn] Pillow not installed; skipping image measurement (pip install Pillow)", file=sys.stderr)
        return html

    container_patterns = [
        r'<section data-container="gallery-scroll"[^>]*>.*?</section>\s*</section>',
        r'<section data-container="longimage-scroll"[^>]*>.*?</section>\s*</section>',
        r'<section data-container="duo"[^>]*>.*?</section>\s*</section>',
    ]
    ...
    def replace_img(match: re.Match) -> str:
        ...
        if re.search(r'\bstyle\s*=', full_tag):
            return full_tag
        ...
        if src.startswith(("http://", "https://", "//")):
            pct = 70
            print(f"[warn] external image, using 70%: {src}", file=sys.stderr)
        else:
            ...
            try:
                ...
                ratio = w / h
                pct = width_pct_from_ratio(ratio)
                if pct is None:
                    print(f"[warn] very tall image (ratio={ratio:.2f}), using 45%: {src}", file=sys.stderr)
                    pct = 45
            except (FileNotFoundError, UnidentifiedImageError, OSError, ValueError) as e:
                print(f"[warn] cannot measure {src} ({type(e).__name__}), using 70%", file=sys.stderr)
                pct = 70
        style = f"width:{pct}%;display:block;margin:12px auto;border-radius:6px"
        # 把 style 插在第一个属性位置，以便下游 step5 的 <img(?!\s+style) 负向前瞻能识别并跳过
        return '<img style="' + style + '"' + full_tag[4:]
...
def process_fenced_containers(text: str) -> str:
    ...
    container_re = re.compile(
        r"^:::(dialogue|gallery|longimage|duo|stat|timeline|steps|compare|quote)"
        r"(?:\[([^\]]*)\])?\s*$"
    )
    ...
    while i < len(lines):
        ...
        if container_match:
            ...
            elif container_type == "duo":
                inner_html = md_to_html(inner_text)
                result.append(
                    f'<section data-container="duo">'
                    f'<p data-container="duo-title">{container_title}</p>'
                    f'<section data-container="duo-inner">'
                    f'{inner_html}'
                    f'</section></section>'
                )
...
def _inject_container_styles(html: str, theme: dict) -> str:
    ...
    dialogue_text = "font-size:16px;color:#333;line-height:1.7;margin:0"
    ...
    gallery_scroll = "text-align:center;font-size:0;letter-spacing:0;margin:0 -4px"
    ...
    # gallery 图片：按比例分配宽度，保留完整画面，不裁剪。
    def style_gallery_imgs(m: re.Match) -> str:
        prefix = m.group(1)
        inner = m.group(2)
        suffix = m.group(3)

        if 'data-gallery-item="1"' in inner:
            return prefix + inner + suffix

        items: list[tuple[str, float]] = []
        for tag_m in re.finditer(r'<img\b[^>]*/?>', inner, flags=re.DOTALL):
            tag = tag_m.group(0)
            ratio_m = re.search(r'\bdata-ratio="([0-9.]+)"', tag)
            ratio = float(ratio_m.group(1)) if ratio_m else 1.0
            items.append((_style_gallery_img_tag(tag), ratio))
        return prefix + render_packed_gallery_items(items, flexible_order=False) + suffix
    ...
    # ── duo 容器（两栏并排） ──
    duo_container = "margin:20px 0"
    duo_title = "text-align:center;font-size:14px;color:#999;margin-bottom:12px"
    duo_inner = "display:flex;gap:12px;align-items:flex-start"
    duo_col = "flex:1;min-width:0"
    duo_img = "width:100%;display:block;border-radius:6px;margin:0"
    duo_caption = "text-align:center;font-size:12px;color:#999;margin:6px 0 0 0"

    html = html.replace(
        '<section data-container="duo">',
        f'<section data-container="duo" style="{duo_container}">'
    )
    # 空 duo-title（即使前面 <p> 已被套 style）整条移除
    html = re.sub(
        r'<p[^>]*\bdata-container="duo-title"[^>]*>\s*</p>',
        '',
        html,
    )
    # 非空 duo-title：step 5 已套通用 p 样式，这里叠加 duo-title 专属字号/颜色
    html = re.sub(
        r'<p([^>]*)\bdata-container="duo-title"([^>]*)>',
        lambda m: f'<p style="{duo_title}"{m.group(1)}{m.group(2)} data-container="duo-title">',
        html,
    )
    html = html.replace(
        '<section data-container="duo-inner">',
        f'<section data-container="duo-inner" style="{duo_inner}">'
    )

    def transform_duo(m: re.Match) -> str:
        prefix = m.group(1)
        inner = m.group(2)
        suffix = m.group(3)
        # 两类图片-图说组合：
        #   (1) <p><img .../><em>caption</em></p>    （同段：作者无空行）
        #   (2) <p><img .../></p><p><em>caption</em></p>  （分段：有空行）
        #   (3) <section data-role="img-wrapper"><img ...></section>（wikilink）
        # 提取每张图 + 可选图说，重排成列
        # 第一步：把 (1) 规范化为 (2)（可能中间夹 <br />，em 可能带 style）
        inner = re.sub(
            r'<p[^>]*>\s*(<img\b[^>]*/?>)\s*(?:<br\s*/?>\s*)?<em[^>]*>(.*?)</em>\s*</p>',
            lambda im: f'<p>{im.group(1)}</p><p><em>{im.group(2)}</em></p>',
            inner,
            flags=re.DOTALL,
        )
        # 清理 img 前后的空白（markdown 会留行内换行）
        inner = re.sub(r'(<img\b[^>]*/?>)\s+', r'\1', inner)
        ...
        cols: list[str] = []
        for cm in col_re.finditer(inner):
            img_tag = cm.group(1)
            caption = (cm.group(2) or "").strip()
            if 'style=' in img_tag:
                img_tag = re.sub(r'style="[^"]*"', f'style="{duo_img}"', img_tag)
            else:
                if img_tag.endswith('/>'):
                    img_tag = img_tag[:-2].rstrip() + f' style="{duo_img}" />'
                else:
                    img_tag = img_tag[:-1].rstrip() + f' style="{duo_img}">'
            if not caption:
                alt_m = re.search(r'alt="([^"]*)"', img_tag)
                alt = alt_m.group(1) if alt_m else ""
                caption = alt if 0 < len(alt) <= 20 else ""
            cap_html = f'<p style="{duo_caption}">{caption}</p>' if caption else ""
            cols.append(f'<section style="{duo_col}">{img_tag}{cap_html}</section>')
        return prefix + "".join(cols) + suffix

    html = re.sub(
        r'(<section data-container="duo-inner"[^>]*>)(.*?)(</section>)',
        transform_duo,
        html,
        flags=re.DOTALL,
    )
    ...
    timeline_content = f"font-size:16px;color:#333;line-height:1.7;padding-bottom:16px;border-left:2px solid {accent_20};padding-left:12px;margin-left:5px"
    ...
    steps_content = "font-size:16px;color:#333;line-height:1.7;padding-top:3px"
...
def inject_inline_styles(html: str, theme: dict, skip_wrapper: bool = False, output_dir: Path | None = None) -> str:
    """为每个 HTML 标签注入内联 style 属性

    当提供 output_dir 时，会先调用 measure_and_scale_images 按图片长宽比注入宽度。
    """
    styles = theme["styles"]
    ...
    # === 4.5 测量本地图片像素，按长宽比注入宽度（仅当提供 output_dir）===
    if output_dir is not None:
        html = measure_and_scale_images(html, output_dir)
    ...
    for tag in simple_tags:
        ...
        if tag == "hr":
            ...
            # 宽度由 measure_and_scale_images 决定（测量失败回退 70%）。
            # 这里仅给"未注入 style"的 <img> 套主题默认非宽度样式 + 70% 兜底，保证绝不贴边 100%。
            img_style_nowidth = re.sub(r'width\s*:\s*[^;]+;?', '', s).strip(';').strip()
            fallback_style = f'{img_style_nowidth};width:70%;display:block;margin:12px auto' if img_style_nowidth else 'width:70%;display:block;margin:12px auto'
            html = re.sub(r'<img(?!\s+style) ', f'<img style="{fallback_style}" ', html)
        elif tag == "code":
    ...
    if "wrapper" in style_map and not skip_wrapper:
        if theme.get("layout") == "card":
            ...
            wrapper_s = f'padding:40px 5px;background-color:{card_bg};display:flex;flex-direction:column;align-items:center;gap:40px'
            html = f'<section style="{wrapper_s}">{html}</section>'
        else:
            # 白色背景 wrapper 在微信客户端会产生可见色差，跳过
            wrapper_bg = styles.get("wrapper", {}).get("background_color", "").lower().strip()
            if wrapper_bg not in ("#ffffff", "#fff", "white", "transparent", ""):
...
def _render_single_theme(tid, theme_data, gallery_html, gallery_footnote, output_dir=None):
    """渲染单个主题（用于并行 gallery）"""
    rendered = inject_inline_styles(gallery_html, theme_data, output_dir=output_dir)
...
def format_for_output(content: str, input_path: Path, theme: dict,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    content = convert_html_galleries(content, input_path.parent, output_dir)
    ...
    # wechat: 全套微信兼容处理
    html = inject_inline_styles(html, theme, output_dir=output_dir)
    if footnote_html:
...
def main():
    ...
    html = md_to_html(content)
    # 正文剥离首个 H1：微信页首已原生展示标题，正文重复大标题会吃掉整个首屏，
    # 且带背景色的 H1 块在深色模式下无法被 data-darkmode-bgcolor 覆盖（渐变背景）。
    # 标题通过 article.html 顶部的 <!--TITLE:--> 注释传给 publish.py。
    html = re.sub(r"\A\s*<h1[^>]*>.*?</h1>\s*", "", html, flags=re.DOTALL)
    html, footnote_html = extract_links_as_footnotes(html)
    ...
    if args.gallery:
        ...
        with ThreadPoolExecutor(max_workers=min(8, len(gallery_theme_ids))) as executor:
            futures = {
                executor.submit(
                    ...
                    gallery_html, gallery_footnote, output_dir
                ): tid
        ...
        if AUTO_OPEN and not args.no_open:
            subprocess.Popen(["open", str(gallery_path)])
    ...
    # 保存纯文章 HTML（顶部注释携带标题，供 publish.py 提取）
